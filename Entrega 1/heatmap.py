...
# Crear un dataframe con las columnas numéricas normalizadas
numeric_cols = datosFiltrados.select_dtypes(include=np.number).columns
gdat = datosFiltrados.copy()
gdat[numeric_cols] = gdat[numeric_cols].apply(lambda x: (x - x.mean()) / x.std())
...
sns.heatmap(dist_matrix, cmap="viridis", xticklabels=gdat['genero1'], yticklabels=gdat['genero2'])
